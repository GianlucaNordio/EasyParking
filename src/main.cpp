...
// STEP 5: Minimap creation
    ...
    for(int i = 0; i < baseSequence.size(); i++)
        baseSequenceMinimap.push_back(cv::Mat(MINIMAP_ROWS, MINIMAP_COLS, IMAGE_TYPE_3_CHANNELS, WHITE));
        
    for(int i = 0; i < NUMBER_SEQUENCES; i++) {
        datasetMinimap.push_back(std::vector<cv::Mat>());
        for(int j = 0; j < dataset[i].size(); j++) {
            datasetMinimap[i].push_back(cv::Mat(MINIMAP_ROWS, MINIMAP_COLS, IMAGE_TYPE_3_CHANNELS, WHITE));
        }
    }
