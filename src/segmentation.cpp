...
Segmentation::Segmentation(const std::vector<cv::Mat> &backgroundImages) {
    ...
    const int HISTORY_DEFAULT_VALUE = 5;
    const bool SHADES_DETECTION = true; 
    const int VAR_THRESHOLD = 5;
    //pBackSub = cv::createBackgroundSubtractorMOG2(HISTORY_DEFAULT_VALUE, VAR_THRESHOLD, SHADES_DETECTION);
    //pBackSub = cv::createBackgroundSubtractorKNN(HISTORY_DEFAULT_VALUE, VAR_THRESHOLD, SHADES_DETECTION);
...
void Segmentation::segmentImage(const cv::Mat &image, cv::Mat &outputMask) {
    const int PIXEL_SIZE_LOW_THRESHOLD = 600;   // Threshold for the minumum size of the connected component to be kept
    const int PIXEL_SIZE_HIGH_THRESHOLD = 5000;
    const int CONNECTIVITY_8 = 4;   // 8-connectivity for connectedComponentsWithStats
    ...
    cv::Mat closedMask;
    cv::morphologyEx(outputMask, closedMask, cv::MORPH_OPEN, element);


    // Compute connected components and their statistics
    cv::Mat stats, centroids, labelImage;
    int numLabels = cv::connectedComponentsWithStats(closedMask, labelImage, stats, centroids, CONNECTIVITY_8, CV_32S);
    ...
    for (int i = 1; i < numLabels; ++i) {
        ...
        if (area > PIXEL_SIZE_LOW_THRESHOLD &&  area < PIXEL_SIZE_HIGH_THRESHOLD) {
